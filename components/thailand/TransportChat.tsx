...
export default function TransportChat({ language, destination }: Props) {
  ...
  const getNextStepId = (currentId: string, currentAnswers: Record<string, any>): string | null => {
    ...
    for (let i = currentIndex + 1; i < STEP_SEQUENCE.length; i++) {
      ...
      const type = currentAnswers['rental_type'];

      const isSelfDrive = type && (
        String(type).includes('Self-Drive') || 
        String(type).includes('ขับเอง') || 
        String(type).includes('ကိုယ်တိုင်မောင်း') || 
        String(type).includes('Selbstfahrer') || 
        String(type).includes('Sans chauffeur') ||
        String(type).includes('Sin chofer')
      );

      const isWithDriver = type && (
        String(type).includes('With Driver') || 
        String(type).includes('พร้อมคนขับ') || 
        String(type).includes('ဒရိုင်ဘာနှင့်တကွ') || 
        String(type).includes('Mit Chauffeur') || 
        String(type).includes('Avec chauffeur') || 
        String(type).includes('Con chofer')
      );

      if (nextId === 'self_drive_license' && !isSelfDrive) continue;
      if (nextId === 'self_drive_deposit' && !isSelfDrive) continue;
      if (nextId === 'driver_hours' && !isWithDriver) continue;
      if (nextId === 'driver_language' && !isWithDriver) continue;
  ...
  const triggerAISubmission = async (finalAnswers: Record<string, any>) => {
    ...
    let formattedSummary = `THAILAND VEHICLE RENTAL SURVEY DETAILS (Destination: ${destination}):\n`;
...
3. Suggest the optimal car class and specific package features fitting their survey profile.
4. Highlight important local guidelines (e.g., parking, navigation tips, or chauffeur hours) depending on their Self-Drive/With-Driver selection.
5. Keep the response factual and elegantly structured. At the end, state that our operations desk is standing by to coordinate booking quotes and secure the vehicle.`;
