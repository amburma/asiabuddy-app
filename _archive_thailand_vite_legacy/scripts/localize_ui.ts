...
const uiTranslations: Record<string, VatUI> = {
  ...
  myanmar: {
    ...
    description: "\"VAT Refund for Tourists\" ဆိုင်းဘုတ်များကိုရှာပါ။ အနည်းဆုံး ၂၀၀၀ ဘတ်သုံးပါ။",
    link: "ပိုမိုသိရှိလိုပါက VAT ပြန်အမ်းငွေကိုနှိပ်ပါ"
  },
  spanish: {
  ...
  farsi: {
    ...
    description: "به دنبال تابلوهای 'استرداد مالیات بر ارزش افزوده برای گردشگرانت' باشید. حداقل خرید ۲ هزار بات.",
...
for (const lang of allLanguages) {
  ...
  if (lang === 'english') {
    ...
    // Look for the language block and its footer
    const langPattern = new RegExp(`${lang}: \\{[^}]*footer: \\{[^}]+\\n    \\}`, "s");
    const replacement = `,\n    vatRefund: {\n      title: "${t.title}",\n      description: "${t.description}",\n      link: "${t.link}"\n    }`;
    
    // Check if vatRefund already exists for this language
    const vatPattern = new RegExp(`${lang}: \\{[^}]*vatRefund: \\{[^}]+\\}`, "s");
    
    if (vatPattern.test(content)) {
      // Update existing
      const existingVatRegex = new RegExp(`(${lang}: \\{[^}]*)vatRefund: \\{[^}]+\\}`, "s");
      content = content.replace(existingVatRegex, `$1vatRefund: {\n      title: "${t.title}",\n      description: "${t.description}",\n      link: "${t.link}"\n    }`);
    } else if (langPattern.test(content)) {
      // Insert after footer
      content = content.replace(langPattern, (match) => match + replacement);
    }
  }
}

fs.writeFileSync(i18nPath, content);
console.log("Updated i18n.ts with ALL languages");
