...
import { getBooking, updateBookingStatus, createInvoice } from '../../../src/lib/database';
import { generateInvoicePDF } from '../../../src/services/pdfGenerator';

const operatorBot = new Bot(process.env.OPERATOR_BOT_TOKEN!);
const customerBot = new Bot(process.env.TELEGRAM_BOT_TOKEN!);
...
operatorBot.callbackQuery(/^approve_(.+)$/, async (ctx) => {
  ...
  try {
    ...
    await customerBot.api.sendDocument(
      booking.telegram_id,
...
const handler = webhookCallback(operatorBot, 'std/http');
