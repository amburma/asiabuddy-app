"use client"; // animation နှင့် client-side logic များအတွက် လိုအပ်သည်
...
import { LAWS_DATA, LAWS_PRO_TIP } from '../data/lawsData';
...
export default function LawsGuide({ language }: Props) {
  ...
  const data = LAWS_DATA[language] || LAWS_DATA.english;
  const proTip = LAWS_PRO_TIP[language] || LAWS_PRO_TIP.english;
    ...
    <div className="bg-white min-h-full">
      ...
      <div className="p-8 max-w-4xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          <h2 className="text-2xl font-serif text-[#2d4a3e] italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {data.map((section: any, idx: number) => (
            <motion.div 
              ...
              className="p-6 rounded-3xl border border-[#D4AF37]/10 bg-white space-y-4 hover:border-red-200 transition-all group shadow-sm"
            >
        ...
        {/* Pro Tip Section */}
        <div className="p-8 bg-[#fdfaf3]/50 rounded-3xl border border-[#D4AF37]/20 flex flex-col md:flex-row items-center gap-6">
          <div className="p-4 bg-white rounded-2xl shadow-sm text-[#D4AF37]">
            <ShieldAlert size={32} />
          </div>
          <div className="space-y-2">
            <h4 className="text-xs font-bold uppercase tracking-[0.2em] text-[#2d4a3e]">
