...
import { Send, ShoppingBag, Loader2, MessageSquare } from 'lucide-react';
...
export default function ShoppingChat({ language }: Props) {
  ...
  const handleSend = async (customMessage?: string) => {
    ...
    const contextPrompt = `You are a specialized Thailand Shopping concierge. Help the traveler with specific shopping advice (Luxury malls, night markets like Jodd Fairs, Chatuchak strategies, VAT refund process, bargaining etiquette, etc.). Answer this: ${userMessage}`;
      ...
      <div className="p-4 bg-sacred-bg border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-gray-500 font-medium tracking-tight">Thailand Shopping Advisor</p>
          </div>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="space-y-6 py-4">
            <div className="text-center opacity-70">
              ...
              <p className="text-xs font-serif italic mb-2 tracking-wide text-gray-800">{t.chatTitle}</p>
            </div>
